  ...
  <div class="feature-shell">
    <section class="toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <el-button class="btn-ghost" size="small" @click="add">新增分类</el-button>
        <el-button
          size="small"
    ...
    <el-dialog :show-close="false" :visible.sync="dialogVisible" width="30%" custom-class="system-dialog">
      <p slot="title" class="dialog-card-title">{{ !isEdit ? '新增分类' : '编辑分类' }}</p>
      <div style="padding:0 20px;">
        <span class="dialog-hover">分类名称</span>
        <input class="dialog-input" v-model="form.name" placeholder="分类名称" />
      </div>
...
export default {
    ...
    methods: {
        ...
        async batchDelete() {
            ...
            const confirmed = await this.$swalConfirm({ title: '删除分类', text: '删除后不可恢复，是否继续？', icon: 'warning' });
            if (confirmed) {
                try {
        ...
        add() { this.isEdit = false; this.form = { name: '' }; this.dialogVisible = true; },
